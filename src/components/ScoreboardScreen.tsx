...
function TeamPanel({ label, names, score, color }: { label: string; names: string[]; score: number; color: 'blue' | 'rose' }) {
  const colorClass = color === 'blue' ? 'border-blue-400 bg-blue-950/50' : 'border-rose-400 bg-rose-950/50';
  return (
    <div className={`rounded-3xl border p-5 ${colorClass}`}>
      <div className="flex items-center justify-between gap-4">
        <span className="rounded-full bg-white px-3 py-1 text-sm font-black text-gray-950">{label}팀</span>
        <span className="text-6xl font-black leading-none tabular-nums">{score}</span>
      </div>
      <div className="mt-4 grid gap-2">
        {names.length ? (
          names.map((name) => (
            <div key={name} className="rounded-2xl bg-white/10 px-4 py-3 text-2xl font-black">
              {name}
            </div>
          ))
        ) : (
          <div className="rounded-2xl bg-white/10 px-4 py-3 text-2xl font-black text-white/50">-</div>
        )}
      </div>
    </div>
  );
...
function AdminScoreControls({ match, onSaved }: { match: MatchView; onSaved: () => Promise<void> }) {
  ...
  async function save(status: 'playing' | 'paused' | 'finished') {
    ...
    setMessage(status === 'finished' ? '경기 종료 처리됨' : status === 'paused' ? '일시중지 처리됨' : '경기진행 처리됨');
    await onSaved();
  }
...
export function ScoreboardScreen({ viewOnly = false }: { viewOnly?: boolean }) {
  ...
  const courtGridClass = useMemo(() => {
    if (displayColumns === '1') return 'grid-cols-1';
    if (displayColumns === '2') return 'grid-cols-1 xl:grid-cols-2';
    if (displayColumns === '3') return 'grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3';
    if (displayColumns === '4') return 'grid-cols-1 lg:grid-cols-2 2xl:grid-cols-4';
    if (courts.length <= 1) return 'grid-cols-1';
    if (courts.length === 2) return 'grid-cols-1 2xl:grid-cols-2';
    if (courts.length <= 4) return 'grid-cols-1 xl:grid-cols-2';
    return 'grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3';
  }, [courts.length, displayColumns]);
  ...
  useEffect(() => {
    const initialEventId = viewOnly ? '' : new URLSearchParams(window.location.search).get('event_id') || '';
  ...
  return (
    <main className="min-h-screen bg-gray-950 p-4 text-white md:p-6">
      <header className="mb-5 flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/5 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="mt-4 text-3xl font-black md:text-5xl">{selectedEvent?.title || '진행 중인 모임 없음'}</h1>
          <p className="mt-2 text-lg font-bold text-white/60">{selectedEvent ? `${selectedEvent.event_date} · ${selectedEvent.location || '장소 미지정'}` : '모임관리에서 모임과 대진을 생성하세요.'}</p>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          {canManageScore && (
            ...
            >
              <option value="auto">자동배치</option>
              <option value="1">1행 1열</option>
              <option value="2">1행 2열</option>
              <option value="3">1행 3열</option>
              <option value="4">1행 4열</option>
            </select>
          )}
      ...
      {error && <p className="mb-5 rounded-2xl bg-red-500/20 p-4 font-bold text-red-100">{error}</p>}
      {loading ? (
        <section className="rounded-3xl bg-white/5 p-8 text-center text-2xl font-black">전광판을 불러오는 중입니다.</section>
      ) : (
        <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
          <section className={`grid gap-5 ${courtGridClass}`}>
            {courts.map((court) => (
              <article key={court.court_no} className="rounded-[2rem] border border-white/10 bg-white/5 p-5 shadow-2xl">
                <div className="mb-5 flex items-center justify-between gap-4">
                  <div>
                    <p className="text-sm font-black uppercase tracking-[0.3em] text-emerald-300">Court</p>
                    <h2 className="text-4xl font-black">{court.court_name}</h2>
                  </div>
                  {court.match ? (
                    <div className="text-right">
                      <p className="text-sm font-bold text-white/50">현재 경기</p>
                    ...
                    <span className="rounded-full bg-white/10 px-4 py-2 font-bold text-white/50">배정 경기 없음</span>
                  )}
                </div>
                ...
                {court.match ? (
                  ...
                  <div className="rounded-3xl border border-dashed border-white/20 p-10 text-center text-2xl font-black text-white/40">대기 중</div>
                )}
              </article>
            ))}
          ...
          <aside className="rounded-[2rem] border border-white/10 bg-white/5 p-5">
            <div className="mb-4 flex items-center justify-between">
              ...
              <span className="rounded-full bg-white px-3 py-1 text-sm font-black text-gray-950">{waitingMatches.length}개</span>
            </div>
            <div className="grid gap-3">
              {waitingMatches.map((match) => (
                <div key={match.id} className="rounded-3xl bg-gray-900 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <b className="text-lg">
                      R{match.round_no}-{match.match_no}
                    </b>
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-bold">{match.court_name || `${match.court_no || '-'}코트`}</span>
                  </div>
